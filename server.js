...
async function createOrder(body) {
  ...
  if (method === 'bKash' && !String(cfg.bkash_number||'').trim()) throw new Error('bKash is not configured by the admin.');
  if (method === 'Nagad' && !String(cfg.nagad_number||'').trim()) throw new Error('Nagad is not configured by the admin.');
  if (method === 'Rocket' && !String(cfg.rocket_number||'').trim()) throw new Error('Rocket is not configured by the admin.');
